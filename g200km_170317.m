...
load newBATH200
% newBATH200(1,:) = [];
remove = find(newBATH200(:,3) == -9999);
newBATH200(remove,:) =[];

% create a find and replace for the elevations above sea level
load km200RIM
km200RIM(1,:) = [];
replace = find(km200RIM(:,3) <= 0);
km200RIM(replace,:) = newBATH200(replace,:);
